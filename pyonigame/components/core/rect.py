...
class Rect(EventBase, CoordinateBase, ShapeBase, ColorBase):
    ...
    def __init__(self, x: int, y: int, width: int, height: int, color: Union[tuple[float, float, float], str], border_color: Union[tuple[float, float, float], str] = "transparent", layer=EventBase.Layer.GAME_ELEMENT, event_subscription: Event = Event.NONE, border_radius=0, border_width=5):
